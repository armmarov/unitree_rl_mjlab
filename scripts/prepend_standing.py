...
Adds frames that interpolate from R1's home pose to the first frame
...
# R1 home pose: root position, root quaternion (xyzw), 24 joint angles.
# Matches HOME_KEYFRAME in src/assets/robots/unitree_r1/r1_constants.py.
HOME_ROOT_POS = np.array([0.0, 0.0, 0.76])
HOME_ROOT_QUAT_XYZW = np.array([0.0, 0.0, 0.0, 1.0])  # Identity
HOME_JOINTS = np.array([
  -0.1, 0.0, 0.0, 0.3, -0.2, 0.0,        # Left leg
  -0.1, 0.0, 0.0, 0.3, -0.2, 0.0,        # Right leg
  0.0, 0.0,                                # Waist (roll, yaw)
